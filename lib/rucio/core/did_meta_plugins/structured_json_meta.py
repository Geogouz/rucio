...
class StructuredJSONDidMeta(DidMetaPlugin):
    ...
    def set_metadata_atomic(
        ...
        Example:
            set_metadata_atomic(scope, name, "ab,cd,ef", {"key": "value"})
            # Results in: {"ab": {"cd": {"ef": {"key": "value"}}}}
        """
        if not json_implemented(session=session):
            # Raise an UnsupportedOperation exception if JSON is not implemented in the DB.
        ...
        try:
            ...
            if did_meta_row_entry is None:
                ...
                if session.bind.dialect.name == 'postgresql':
                    # Parse the key path for JSONB operations
                    jsonb_path = '{' + key + '}'
                    ...
                    stmt = (
                        ...
                        .values(
                            structured_meta=func.jsonb_set(
                                func.coalesce(models.DidMeta.structured_meta, func.jsonb('{}')),
                                jsonb_path,
                                func.jsonb(json_lib.dumps(value))
                            )
                        )
                    )
