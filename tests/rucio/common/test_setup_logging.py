...
def test_setup_logging_redirect_env_overrides_config(monkeypatch):
    ...
    monkeypatch.setattr(rucio_logging, "config_get_bool", _mock_config_get_bool({"redirect_stdout_to_stderr": True}))
...
def test_setup_logging_configures_application_logger(monkeypatch):
    ...
    flask_logging_mod = types.ModuleType("flask.logging")
    flask_logging_mod.default_handler = default_handler
    flask_mod.logging = flask_logging_mod
    monkeypatch.setitem(sys.modules, "flask", flask_mod)
    monkeypatch.setitem(sys.modules, "flask.logging", flask_logging_mod)
