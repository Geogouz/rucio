...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    group.addoption(
        ...
        help="Run the selected suite with this database",
    )
    group.addoption(
        "--list-cases",
        action="store_true",
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    suite = os.environ.get("RUCIO_TEST_SUITE")
    python_version = os.environ.get("RUCIO_TEST_PYTHON")
    rdbms = os.environ.get("RUCIO_TEST_RDBMS", "")
    policy = os.environ.get("RUCIO_TEST_POLICY", "")
    if suite is None and python_version is None:
        case = get_case(case_id)
    elif not suite or not python_version:
        raise pytest.UsageError("RUCIO_TEST_SUITE and RUCIO_TEST_PYTHON must be set together")
    else:
        try:
            case = make_case(
                suite,
        ...
        if case.id != case_id:
            raise pytest.UsageError(
                f"RUCIO_TEST_CASE {case_id!r} does not match configured axes {case.id!r}"
            )
    runtime_python = f"{sys.version_info.major}.{sys.version_info.minor}"
    if case.python != runtime_python:
        raise pytest.UsageError(
            f"Case {case.id} requires Python {case.python}, but the container runs {runtime_python}"
        )
    config.stash[case_key] = _resolve_policy_paths(case, SOURCE_ROOT)
...
def pytest_cmdline_main(config: pytest.Config) -> "Optional[int]":
    ...
    if config.getoption("looponfail", False) and case.suite == "multi_vo":
        raise pytest.UsageError(
...
def _run_cases(
    ...
    if (
        ...
        and len(server_runtimes) > 1
        and not all(
            os.environ.get(
                f"RUCIO_TEST_IMAGE_{runtime.replace('-', '_').upper()}"
            )
            for runtime in server_runtimes
        )
    ):
        raise pytest.UsageError(
            "Multi-runtime suites require one RUCIO_TEST_IMAGE_<runtime> "
            "variable per runtime instead of RUCIO_TEST_IMAGE"
        )
    ...
    workers = config.getoption("xdist_workers")
    pytest_args = _normalize_xdist_args(config, pytest_args, workers)
    if config.getoption("looponfail", False) and (
...
def resolve_requested_cases(
    ...
    python_version = config.getoption("python_version")
    rdbms = config.getoption("rdbms")
    ...
    if case_id and (python_version or rdbms):
        raise pytest.UsageError("--python and --rdbms require --suite")
    ...
    if suite == "all":
        if rdbms or policy:
            raise pytest.UsageError("--rdbms and --policy cannot be combined with --suite=all")
        return tuple(
            _resolve_policy_paths(case, root_path)
            for name in SUITE_DEFINITIONS
            for case in iter_suite_cases(name, python=python_version)
        )

    try:
        candidates = tuple(iter_suite_cases(
            suite,
