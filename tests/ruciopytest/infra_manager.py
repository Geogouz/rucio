...
class InfraManager:
    ...
    def setup(self) -> None:
        ...
        if not second_vo:
            if self.environment.get("RUCIO_PYTEST_INNER") == "1" and not database_ready:
    ...
    def prepare_runtime(self) -> None:
        """Prepare state which is local to one execution container."""
        self._ensure_memcached(flush=False)
        self._cleanup_temporary_state()
        if self.case.suite == "integration":
            self._configure_integration()
    ...
    def _upgrade_database(self) -> None:
        self._run((
            "alembic",
            "-c",
            str(self.rucio_home / "etc/alembic.ini"),
            "upgrade",
            "head",
        ))
    ...
    @staticmethod
    def _ensure_memcached(*, flush: bool = True) -> None:
        try:
...
def main(arguments: "Optional[Sequence[str]]" = None) -> int:
    ...
    manager = InfraManager(case, keep_db=options.keep_db)
