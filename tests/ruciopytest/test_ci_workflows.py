...
def test_local_runtime_builds_initialize_buildx(filename: str, job: str) -> None:
    workflow = yaml.safe_load((WORKFLOW_DIR / filename).read_text())
    steps = {step.get("name"): step for step in workflow["jobs"][job]["steps"]}

    assert (
        steps["Set up Docker Buildx"]["if"]
        == steps["Build runtime image locally"]["if"]
        == "fromJSON(needs.runtime_images.outputs.builds)[matrix.case.runtime]"
    )


def test_toolbox_contract_uses_branch_parent() -> None:
    workflow = yaml.safe_load((WORKFLOW_DIR / "developer_toolbox.yml").read_text())
    checkout = next(
