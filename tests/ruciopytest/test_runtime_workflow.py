...
def test_runtime_images_preserve_target_and_platform_identity() -> None:
    workflow = WORKFLOW.read_text()

    assert "target: .runtime_target" in workflow
    assert "platform: .runtime_platform" in workflow
    assert 'echo "TARGET=${TARGET}"' in workflow
    assert 'echo "PLATFORM=${PLATFORM:-native}"' in workflow
    assert '--target "$TARGET"' in workflow
    assert 'PLATFORM_ARGS+=(--platform "$PLATFORM")' in workflow
    assert "rucio-dev-runtime:${IMAGE_KEY}-buildcache" in workflow
