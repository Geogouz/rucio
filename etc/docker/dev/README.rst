...
Develop the Rucio WebUI
~~~~~~~~~~~~~~~~~~~~~~~

The toolbox can run the Rucio backend and an existing `Rucio WebUI
<https://github.com/rucio/webui>`__ Git worktree as one foreground development
environment. Node.js and npm must be available on the host. From the Rucio
checkout, pass the path of any existing WebUI Git worktree::

    ./tools/dev doctor
    ./tools/dev webui ../webui

The command installs WebUI dependencies with ``npm ci`` when ``node_modules``
is missing or when ``package.json``, ``package-lock.json``, Node.js, npm, or the
host platform changed. Later starts reuse the installation while those inputs
remain unchanged. No manual dependency command is needed.

The command then starts the base Rucio services, prepares reusable WebUI
catalogue fixtures, publishes the Flask API on ``http://127.0.0.1:8080``, and
starts the WebUI on ``http://localhost:3000``. Log in with username ``rucio``
and password ``rucio``. The account and virtual organization are ``rucio`` and
``def``. Physical transfer services are not needed for these catalogue
fixtures. WebUI configuration is injected into the host process. Existing
``.env.local`` files are not modified.

The supplied path must be a valid Rucio WebUI Git worktree with a
``package-lock.json``. The toolbox uses its current branch and working tree. It
does not create, remove, or switch worktrees or branches and does not modify
tracked files. It writes only ignored generated content in ``node_modules`` and
the Next.js build cache. ``npm ci`` replaces ``node_modules`` when the recorded
dependency inputs change. Use a different existing WebUI worktree when
retaining another WebUI branch alongside the current one.

Only one combined environment can own the fixed loopback ports at a time. The
command fails before starting Docker when port 8080 or 3000 is already in use.
It does not terminate or adopt an existing process. Stop the previous owner and
retry. Run ``./tools/dev down`` when an interrupted toolbox process left the
checkout's port mapping behind. Press **Ctrl+C** to stop both development
processes. The checkout-scoped Docker volumes and runtime images remain
...
.. list-table::
   ...
   * - Persistent workspace
     ...
     - ``up``, ``shell``, ``run``, ``observe``, and non-test debug and profile
       commands use the content-fingerprinted AlmaLinux runtime. They do not
...
``python:<minor>-slim-bookworm`` tag. Service-backed Python 3.9 uses AlmaLinux
distribution packages. Service-backed Python 3.10 through 3.13 uses exact
checksum-verified CPython source releases. The current pins are 3.10.20,
3.11.15, 3.12.13, and 3.13.14. The Dockerfile is the source of truth when these
patch releases change.
...
interpreter, and compiles ``mod_wsgi`` against that interpreter. It then
...
service-backed test commands issue a local Buildx build. A cold Python 3.10
...
Elapsed time depends on whether the runtime is already built and on Docker's
CPU, memory, disk, and registry access. A warm focused test should complete in
seconds. A cold source-built runtime can take tens of minutes before pytest
starts, while a complete database case adds service and catalogue setup time.
...
reuses the matching native AlmaLinux 9/Python 3.10 runtime, creates a fresh
