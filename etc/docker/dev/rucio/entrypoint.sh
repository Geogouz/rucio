...
if [ -z "$RDBMS" ]; then
    ...
    if [ ! -d /usr/lib/oracle ]; then
        ...
        LIBDIR="$(rpm -ql oracle-instantclient-basiclite | grep -E '/usr/lib/oracle/.*/client64/lib$' | head -n1)"
        if [ -z "$LIBDIR" ]; then
            echo "[ERROR] Could not locate Instant Client lib dir (rpm -ql)."
            exit 1
        fi
        echo "$LIBDIR" > /etc/ld.so.conf.d/oracle.conf
        ldconfig
