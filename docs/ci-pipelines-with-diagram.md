...
```mermaid
flowchart TB
  subgraph Triggers [GitHub Actions event sources]
    direction TB
    PR[Pull Request]
    Push[Push]
  ...
  subgraph ReusableRuntime [Reusable runtime_images workflow]
    direction TB
    RuntimeImages["runtime_images.yml<br>(outputs: py39_image, py310_image, build_locally)"]
  end

  Push --> RuntimeImages
  Schedule --> RuntimeImages
  Dispatch --> RuntimeImages

  ATWF["Run all the autotests<br>(.github/workflows/autotest.yml)"]
  PR --> ATWF
  Push --> ATWF
  Schedule --> ATWF

  subgraph ATJobs [Autotest workflow jobs]
    direction TB
    AutoHeader[Add header lint]
    AutoRuff[python_ruff]
    AutoAnnotations["Check Python Type Annotations<br>(PR/push non-release)"]
    AutoPyright["Python type check<br>(Pyright, PR/push non-release)"]
    AutoRuntime["Build Runtime Images<br>(PR/push only)"]
    AutoSetup["setup job<br>set_branch_output_env.py + matrix_parser.py"]
    AutoTest["test job<br>run_tests.py"]
  end
  ATWF --> AutoHeader
  ATWF --> AutoRuff
  ATWF --> AutoAnnotations
  ATWF --> AutoPyright
  ATWF --> AutoRuntime
  ATWF --> AutoSetup
  AutoRuntime --> AutoTest
  AutoSetup --> AutoTest
  AutoRuntime -. workflow_call .-> RuntimeImages
  RuntimeImages -->|py39/py310 tags<br>build_locally flag| AutoTest

  VOWF["VO-specific tests<br>(.github/workflows/vo_tests.yml)"]
  PR --> VOWF
  Push --> VOWF
  Schedule --> VOWF
  Dispatch --> VOWF

  subgraph VOJobs [VO tests workflow jobs]
    direction TB
    VORuntime["Build Runtime Images<br>(reusable workflow)"]
    VOSetup["setup job<br>set_branch_output_env.py + votest_helper.py"]
    VOTest["test job<br>run_tests.py"]
  end
  VOWF --> VORuntime
  VOWF --> VOSetup
  VORuntime --> VOTest
  VOSetup --> VOTest
  VORuntime -. workflow_call .-> RuntimeImages
  RuntimeImages -->|py39 tag<br>build_locally flag| VOTest

  INTF["End-to-End integration tests<br>(.github/workflows/integration_tests.yml)"]
  PR --> INTF
  Push --> INTF
  Schedule --> INTF

  subgraph INTJobs [Integration workflow jobs]
    direction TB
    INTRuntime["Build Python 3.9 runtime image<br>(reusable workflow)"]
    INTSetup["setup job<br>set_branch_output_env.py + matrix_parser.py"]
    INTExec["integration-tests job<br>compose orchestration"]
  end
  INTF --> INTRuntime
  INTF --> INTSetup
  INTRuntime --> INTExec
  INTSetup --> INTExec
  INTRuntime -. workflow_call .-> RuntimeImages
  RuntimeImages -->|py39 tag<br>build_locally flag| INTExec

  ICWF["Runtime image cache builder<br>(.github/workflows/imagecache.yml)"]
  Schedule --> ICWF
  Dispatch --> ICWF

  subgraph ICJobs [Image cache workflow jobs]
    direction TB
    ICSetup["setup job<br>grabrelease.py"]
    ICAutotest[build_autotests job]
    ICIntegration[build_integration_tests job]
  end
  ICWF --> ICSetup
  ICSetup --> ICAutotest
  ICSetup --> ICIntegration
  ICAutotest --> GHCR
  ICIntegration --> GHCR

  CleanWF["Cleanup runtime images<br>(.github/workflows/cleanup_runtime_images.yml)"]
  Schedule --> CleanWF
  Dispatch --> CleanWF

  subgraph CleanJobs [Cleanup workflow jobs]
    direction TB
    CleanUntagged["cleanup-untagged<br>GHCR API sweep"]
    CleanTagged["cleanup-tagged<br>preserve buildcache tags"]
  end
  CleanWF --> CleanUntagged
  CleanWF --> CleanTagged

  UnitWF["Unit tests<br>(.github/workflows/unit_tests.yml)"]
  PR --> UnitWF
  Push --> UnitWF
  Schedule --> UnitWF
  UnitJob[pytest matrix 3.9–3.12]
  UnitWF --> UnitJob

  subgraph Helpers [Shared helpers & execution]
    direction TB
    BranchEnv["tools/github/workflow/set_branch_output_env.py<br>resolve branch output"]
    MPA["tools/test/matrix_parser.py<br>→ JSON matrices"]
    VOT["tools/test/votest_helper.py<br>VO matrix + overrides"]
    MergeConfigs["tools/merge_rucio_configs.py<br>config overlay merges"]
    BI["tools/test/build_images.py<br>Build/push cache images"]
    RT["tools/test/run_tests.py<br>orchestrator"]
    Entry["etc/docker/dev/rucio/entrypoint.sh<br>container entrypoint"]
    TestSh["tools/test/test.sh<br>suite dispatcher"]
    RunTestsSh["tools/run_tests.sh<br>DB reset + bootstrap + pytest"]
    MultiVO["tools/run_multi_vo_tests_docker.sh<br>multi-VO bootstrap"]
    CheckSyntax["tools/test/check_syntax.sh<br>Ruff runner"]
    IgnoreTool["tools/test/ignoretool.py<br>client Ruff selection"]
    ResetDB["tools/reset_database.py"]
    Alembic["tools/alembic_migration.sh"]
    Bootstrap["tools/bootstrap_tests.py"]
    SyncRSE["tools/sync_rses.py"]
    SyncMeta["tools/sync_meta.py"]
    ActivateRSE["tools/docker_activate_rses.sh"]
    Pytest["tools/pytest.sh"]
    subgraph Paths [run_tests.py execution paths]
      direction LR
      Single["RUN_HTTPD=false<br>single container"]
      ComposePath["RUN_HTTPD=true<br>dev docker-compose stack"]
    end
    RT --> Single
    RT --> ComposePath
    Single --> TestSh
    ComposePath --> Entry
    ComposePath --> TestSh
    Entry -. overlays via .-> MergeConfigs
    MergeConfigs --> TestSh
    TestSh -->|syntax| CheckSyntax
    TestSh -->|client_syntax| IgnoreTool
    IgnoreTool --> CheckSyntax
    TestSh -->|client, remote_dbs & sqlite| RunTestsSh
    TestSh -->|SUITE=votest| VOT
    VOT -->|tests via -p| RunTestsSh
    TestSh -->|multi_vo| MultiVO
    RunTestsSh --> ResetDB --> Alembic --> Bootstrap --> SyncRSE --> SyncMeta --> Pytest
    SyncMeta -. optional .-> ActivateRSE -. optional .-> Pytest
    MultiVO -. overlays via .-> MergeConfigs
    MultiVO --> ResetDB
    MultiVO --> Alembic
    MultiVO --> Bootstrap
    MultiVO --> SyncRSE
  ...
  ComposePath --> ComposeFile[etc/docker/dev/docker-compose.yml]
  INTExec --> ComposeFile
  INTExec --> RunTestsSh
  INTExec --> Pytest
  ...
  ICIntegration --> MPA
  ICIntegration --> BI
  ICIntegration --> RC[rucio/containers repo]
  AutoSetup --> BranchEnv
  AutoSetup --> MPA
  INTSetup --> BranchEnv
  INTSetup --> MPA
  ...
  RuntimeImages --> GHCR[(GitHub Container Registry)]
  GHCR --> AutoTest
  GHCR --> VOTest
  GHCR --> INTExec
  BI --> GHCR
