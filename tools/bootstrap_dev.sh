...
function bootstrap_dev::log_selections() {
  ...
  local use_ci_runtime="$(bootstrap_config::get "$cfg_name" use_ci_runtime)"
  local ci_runtime_python="$(bootstrap_config::get "$cfg_name" ci_runtime_python)"
  local profile_count="$(bootstrap_config::profile_count "$profiles_name")"
  local selected_test_description="$(bootstrap_config::get "$cfg_name" selected_test_description)"
  ...
  if [[ "$use_ci_runtime" == "true" ]]; then
    if [[ -n "$selected_test_description" ]]; then
      echo "CI runtime image: runtime.Dockerfile (Python ${ci_runtime_python}) auto-selected for '$selected_test_description'."
    else
      echo "CI runtime image: will build runtime.Dockerfile for Python ${ci_runtime_python}."
    fi
  fi

  if [[ -n "$selected_test" ]]; then
    if [[ -n "$selected_test_description" ]]; then
      echo "Test selection: #$selected_test - $selected_test_description"
    else
...
# bootstrap_dev::auto_configure_test_environment <config_var> <profiles_var>
# ----------------------------------------------
# When a test is selected, detect its runtime requirements and compose
# profiles so the developer environment mirrors the CI setup.
#
function bootstrap_dev::auto_configure_test_environment() {
  local cfg_name="$1"
  local profiles_name="$2"

  local selected_test
  selected_test="$(bootstrap_config::get "$cfg_name" selected_test)"
  if [[ -z "$selected_test" ]]; then
    return
  fi

  local pytest_filter
  pytest_filter="$(bootstrap_config::get "$cfg_name" pytest_filter)"
  local reuse_autotest
  reuse_autotest="$(bootstrap_config::get "$cfg_name" reuse_autotest_images)"

  bootstrap_tests::configure "$selected_test" "$pytest_filter" "$reuse_autotest"
  bootstrap_tests::gather

  local test_desc
  test_desc="$(bootstrap_tests::description_for_test "$selected_test")"
  if [[ -z "$test_desc" ]]; then
    return
  fi
  bootstrap_config::set "$cfg_name" selected_test_description "$test_desc"

  local runtime_python
  runtime_python="$(bootstrap_tests::python_for_test "$selected_test")"
  local image_identifier
  image_identifier="$(bootstrap_tests::image_identifier_for_test "$selected_test")"
  local should_start_profiles="false"
  if [[ -n "$runtime_python" ]]; then
    bootstrap_config::set "$cfg_name" use_ci_runtime "true"
    bootstrap_config::set "$cfg_name" ci_runtime_python "$runtime_python"
    bootstrap_config::set "$cfg_name" skip_pull_base_services "true"
    should_start_profiles="true"
  elif [[ "$image_identifier" == "integration-test" ]]; then
    runtime_python="3.9"
    bootstrap_config::set "$cfg_name" use_ci_runtime "true"
    bootstrap_config::set "$cfg_name" ci_runtime_python "$runtime_python"
    bootstrap_config::set "$cfg_name" skip_pull_base_services "true"
  ...
  local -a auto_profiles=()
  bootstrap_tests::collect_profiles_for_test "$selected_test" auto_profiles
  if (( ${#auto_profiles[@]} > 0 )); then
    should_start_profiles="true"
  fi

  if [[ "$should_start_profiles" == "true" ]]; then
    bootstrap_config::set "$cfg_name" any_profile_arg "true"
    if (( ${#auto_profiles[@]} > 0 )); then
      bootstrap_config::set_profiles "$cfg_name" "$profiles_name" "${auto_profiles[@]}"
    else
      bootstrap_config::set_profiles "$cfg_name" "$profiles_name"
    fi
  fi
}
...
Other:
  -t, --test <N>        Run test number N after bootstrap. The required Compose profiles
                        are derived automatically, so -p/--profile remains unavailable.
...
Notes:
  ...
  4) --filter/-f requires --test/-t.
  5) --cache-use/-c must accompany either --test/-t or --profile/-p.
  6) --expose-ports merges docker-compose.ports.yml so containers expose services on 127.0.0.1.
  7) Selecting a test automatically builds the matching CI runtime image and starts the required profiles.
  8) Running tests is destructive: the dev database volume is removed to ensure a clean state.
  9) The script keeps an 'upstream' remote pointing at rucio/rucio; missing or diverging remotes are fixed before checkout.
USAGE
}
...
bootstrap_dev::auto_configure_test_environment BOOTSTRAP_CONFIG BOOTSTRAP_PROFILES
