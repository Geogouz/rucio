...
from .webui import WEBUI_ACCOUNT, WEBUI_IDENTITY, WEBUI_PASSWORD, checkout_root, ensure_dependencies, process_environment
...
def parser() -> argparse.ArgumentParser:
    ...
    webui = subparsers.add_parser("webui", help="Run Rucio and a local WebUI checkout")
    webui.add_argument("checkout", type=Path, help="Path to an existing Rucio WebUI Git worktree")
...
def _workspace_command(
    ...
    *,
    ...
    if target == "daemon":
        ...
        profiles = ()
        command = _api_server_command(values)
    else:
        if not values:
...
def _api_server_command(arguments: "Sequence[str]" = ()) -> list[str]:
    return [
        "python",
        "-m",
        "flask",
...
def _ensure_port_available(port: int, service: str) -> None:
    with socket.socket() as listener:
        try:
            listener.bind(("127.0.0.1", port))
        except OSError as error:
            raise RuntimeError(f"{service} cannot start because 127.0.0.1:{port} is already in use") from error
...
def _wait_for_http(
    ...
    while time.monotonic() < deadline:
        if process.poll() is not None:
            raise RuntimeError(f"{service} exited with code {process.returncode} before becoming ready")
        try:
...
def _wait_for_webui_processes(processes: "Sequence[tuple[str, subprocess.Popen[bytes]]]") -> int:
    while True:
        for name, process in processes:
            returncode = process.poll()
            if returncode is not None:
                print(f"{name} exited with code {returncode}.", file=sys.stderr)
                return returncode
        time.sleep(0.25)


def webui_command(root: Path, checkout: Path) -> int:
    webui_root = checkout_root(checkout)
    node = shutil.which("node")
    if node is None:
        raise RuntimeError("Required executable not found: node")
    npm = shutil.which("npm")
    if npm is None:
        raise RuntimeError("Required executable not found: npm")

    with workspace_lock(root):
        _ensure_port_available(API_PORT, "Rucio API")
        _ensure_port_available(WEBUI_PORT, "Rucio WebUI")
        ensure_dependencies(webui_root, node, npm)
        project = ComposeProject(root, api=True)
        ...
        try:
            ...
            print("Starting the Rucio API for WebUI development.", flush=True)
            backend = subprocess.Popen(  # noqa: S603
                project.command("exec", "--no-TTY", "workspace", *_api_server_command()),
                cwd=root,
                env=project.environment,
                start_new_session=os.name == "posix",
            )
            processes.append(("Rucio API", backend))
            _wait_for_http(backend, f"http://127.0.0.1:{API_PORT}/ping", "Rucio API")

            print(f"Starting the Rucio WebUI from {webui_root}.", flush=True)
            frontend = subprocess.Popen(  # noqa: S603
                (npm, "run", "dev", "--", "--hostname", "127.0.0.1", "--port", str(WEBUI_PORT)),
                cwd=webui_root,
                env=process_environment(os.environ, secrets.token_hex(32)),
                start_new_session=os.name == "posix",
            )
            processes.append(("Rucio WebUI", frontend))
            _wait_for_http(frontend, f"http://127.0.0.1:{WEBUI_PORT}/auth/login", "Rucio WebUI")

            print("Rucio WebUI development environment ready.", flush=True)
            print(f"WebUI:   http://localhost:{WEBUI_PORT}", flush=True)
            print(f"API:     http://127.0.0.1:{API_PORT}", flush=True)
            print(f"Username: {WEBUI_IDENTITY}", flush=True)
            print(f"Password: {WEBUI_PASSWORD}", flush=True)
            print(f"Account:  {WEBUI_ACCOUNT}", flush=True)
            print("VO:       def", flush=True)
            return _wait_for_webui_processes(processes)
        finally:
...
def _dispatch(args: argparse.Namespace, root: Path) -> int:
    ...
    if args.command == "webui":
        return webui_command(root, args.checkout)
