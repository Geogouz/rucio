...
# bootstrap_common::array_assign <array_var> <index> <value>
# ----------------------------------------------
# Assign a value to an array element using only Bash 3 compatible features.
#
function bootstrap_common::array_assign() {
  local array_name="$1"
  local index="$2"
  local value="$3"
  ...
  eval "${array_name}[${index}]=${quoted_value}"
}
