...
function bootstrap_config::init() {
  ...
  bootstrap_config::set "$config_name" selected_test_description ""
  ...
  bootstrap_config::set "$config_name" skip_pull_base_services "false"
  ...
  bootstrap_config::set "$config_name" use_ci_runtime "false"
  bootstrap_config::set "$config_name" ci_runtime_python ""
  bootstrap_config::set "$config_name" ci_runtime_image ""
...
function bootstrap_config::set_profiles() {
  ...
  local value
  for value in "$@"; do
    bootstrap_common::array_assign "$profiles_name" "$count" "$value"
    count=$((count + 1))
  done
...
function bootstrap_config::copy_profiles() {
  ...
  for ((idx = 0; idx < length; idx++)); do
    ...
    bootstrap_common::array_assign "$target_name" "$idx" "$value"
  done
}
