...
# bootstrap_dev_compose::prepare_runtime_image <config_var>
# ----------------------------------------------
# Build the CI runtime image locally when requested and register it in the
# configuration store so that docker-compose can reuse it.
#
function bootstrap_dev_compose::prepare_runtime_image() {
  local cfg_name="$1"

  local use_ci_runtime
  use_ci_runtime="$(bootstrap_config::get "$cfg_name" use_ci_runtime)"
  if [[ "$use_ci_runtime" != "true" ]]; then
    return
  fi

  local python_minor
  python_minor="$(bootstrap_config::get "$cfg_name" ci_runtime_python)"
  if [[ -z "$python_minor" ]]; then
    bootstrap_common::error "Unable to determine the Python version for the CI runtime image."
    bootstrap_common::error "The selected test matrix entry is missing its PYTHON attribute."
    exit 1
  fi
  if [[ "$python_minor" != "3.9" && "$python_minor" != "3.10" ]]; then
    bootstrap_common::error "The CI runtime image currently supports only Python 3.9 or 3.10."
    exit 1
  fi

  local repo_root
  repo_root="$(bootstrap_config::get "$cfg_name" repo_root)"
  local dry_run
  dry_run="$(bootstrap_config::get "$cfg_name" dry_run_flag)"

  local image_tag
  image_tag="rucio-dev-ci-runtime:py${python_minor}"
  bootstrap_config::set "$cfg_name" ci_runtime_image "$image_tag"
  bootstrap_config::set "$cfg_name" skip_pull_base_services "true"

  local -a build_cmd
  build_cmd=(docker build --build-arg "PYTHON=${python_minor}" --target final \
             -f etc/docker/test/runtime.Dockerfile -t "$image_tag" .)

  if [[ "$dry_run" != "0" ]]; then
    echo ">>> [dry-run] Would build CI runtime image: ${build_cmd[*]}"
    return
  fi

  echo ">>> Building local CI runtime image '$image_tag' (Python ${python_minor})."
  (
    cd "$repo_root"
    "${build_cmd[@]}"
  )
...
function bootstrap_dev_compose::run() {
  ...
  local skip_pull_base_services="$(bootstrap_config::get "$cfg_name" skip_pull_base_services)"
  ...
  bootstrap_dev_compose::prepare_runtime_image "$cfg_name"
  ...
  local runtime_image
  runtime_image="$(bootstrap_config::get "$cfg_name" ci_runtime_image)"
  ...
  if [[ -n "$runtime_image" ]]; then
    export RUCIO_CI_RUNTIME_IMAGE="$runtime_image"
    compose_files+=(--file docker-compose.ci-runtime.yml)
  fi

  local -a up_extra_args=()
  local skip_pull_any="false"
  if [[ "$skip_pull_for_profiles" == "true" || "$skip_pull_base_services" == "true" ]]; then
    skip_pull_any="true"
  fi
  if [[ "$skip_pull_any" == "true" ]] && bootstrap_docker::is_compose_v2; then
  ...
  if (( profile_count == 0 )); then
    ...
    if [[ "$skip_pull_base_services" != "true" && "$skip_pull_for_profiles" != "true" ]]; then
      if [[ "$dry_run" != "0" ]]; then
      ...
      if [[ "$use_ci_runtime" == "true" ]]; then
        echo ">>> CI runtime image selected: skipping 'docker compose pull' for base services."
      else
    ...
    if [[ "$skip_pull_for_profiles" != "true" && "$skip_pull_base_services" != "true" ]]; then
      ...
      if [[ "$use_ci_runtime" == "true" ]]; then
        echo ">>> CI runtime image selected: skipping 'docker compose pull' for profiled services."
      else
        echo ">>> Cache-use enabled: skipping 'docker compose pull' (using locally cached images)."
      fi
    fi
