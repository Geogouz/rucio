...
- `-t, --test <N>` – run test number `N` after bootstrapping. The script derives
  the needed Compose profiles from the matrix entry, so `--profile/-p` is not
  required and remains incompatible.
...
5. `--expose-ports` merges `docker-compose.ports.yml` and binds services on
   `127.0.0.1`.
6. Selecting a test automatically builds the matching CI runtime image and activates the Compose profiles declared in the matrix entry.
7. Tests remove the `dev_vol-ruciodb-data` volume before execution.
8. The script keeps an `upstream` remote pointing at `rucio/rucio`; missing or
...
15. Apply a pytest filter to the default suite:
   ...
   ```
16. Run a matrix entry (builds the CI runtime image and starts the required profiles automatically):
   ```bash
   ./tools/bootstrap_dev.sh --test 9
   ./tools/bootstrap_dev.sh -t 9
   ```
   Selecting the integration workflow (`--test 18`) follows the same pattern and enables the storage, externalmetadata, and IAM profiles for you.
